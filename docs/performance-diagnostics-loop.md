...
- `~/workspace/large-repo`: 28 local branches, 10 worktrees.
- `~/workspace/remote-heavy-repo`: 1 local branch, 16,604 remote refs.
...
| --- | ---: | --- | ---: | --- |
| `large-repo --branches` | 23.90s | `git::branch::list_branches` | 23.3s | `collect_branch_metadata` 13.6s; `detect_merged_branches` 9.70s |
| `large-repo --worktrees` | 28.18s | `git::worktree::enrich_worktrees_worker` | 28.1s | 10 worktrees processed serially; most `git status --porcelain` calls were 3.5-4.0s |
| `remote-heavy-repo --remotes` | stopped | `git::branch::spawn_remote_enricher` | open | `list_remote_branches_phase1` closed in 255ms, then the serial 16,603-branch remote enricher did not finish within the bounded run |
...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| `large-repo --worktrees` | 28.18s | 17.74s | -37.0% | 28.1s | 17.4s | Improvement accepted; child spans overlap now, so compare wall time and parent worker span only |
| `large-repo --branches` | 23.90s | 22.58s | -5.5% | 23.3s | 22.4s | Guardrail only; code path is not touched by this change |
| `remote-heavy-repo --remotes` | stopped | 60s cap | n/a | open | open | Guardrail only; remote enricher remains the next large bottleneck candidate |
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `remote-heavy-repo --remotes` | 60.01s cap | `git::branch::spawn_remote_enricher` | open | `list_remote_branches_phase1` closed in 1.48s; no output rows were produced before the cap |
...
| --- | ---: | ---: | ---: | --- |
| `remote-heavy-repo --remotes` | 60.01s cap | 60.02s cap | no improvement | No output rows; no closed aggregate worker span; `time` reported much higher CPU/system time (`user 117.39`, `sys 193.80`) |
...
Manual probes against `~/workspace/remote-heavy-repo` completed
...
| Repo / view | Before | After | Delta | Evidence |
| --- | ---: | ---: | ---: | --- |
| `remote-heavy-repo remote_enricher_worker` | >60s | 19.0s | accepted | `result_count=16602`, `ahead_behind_count=16604`, `merged_count=546`, `missing_ahead_behind_count=0` |
| `remote-heavy-repo --remotes` full dump | 60s cap | 60s cap | still capped | The bottleneck moved to `squash_candidate` / `is_squash_merged`: 648 candidates consumed 39.8s before the cap; `spawn_squash_checker` had `candidate_count=16056` |
| `large-repo --branches` guardrail | 23.90s baseline | 24.23s | within variance | Guardrail only; branch path is not touched by this change |
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `remote-heavy-repo --remotes` | 60.01s cap | `git::squash_loader::squash_candidate` | 39.85s total busy | After remote enrichment closed in 19.0s, the serial squash checker completed 648 real squash checks before the cap; mean 61.5ms, p95 80.5ms |
...
| Repo / view | Before | After | Delta | Evidence |
| --- | ---: | ---: | ---: | --- |
| `remote-heavy-repo --remotes` full dump | 60.01s cap | 60.02s cap | no improvement | Output remained empty before the cap |
...
The local branch baseline was the latest `large-repo --branches` guardrail after
Iteration 3:
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `large-repo --branches` | 24.23s | `git::branch::collect_branch_metadata` | 14.0s | `collect_branch_metadata_merge_base` was 13.85s; `collect_branch_metadata_ahead_behind_graph` was only 98.2ms |
...
| Repo / view | Before | After | Delta | Evidence |
| --- | ---: | ---: | ---: | --- |
| `large-repo --branches` full dump | 24.23s | 22.83s | not accepted | The apparent wall-time drop was dominated by merge-base variance, not the changed ahead/behind path |
...
| `remote-heavy-repo --remotes` full dump | 60s cap | 60s cap | no improvement | Guardrail remained capped; output stayed empty |
...
For `large-repo --branches`, the data points to
...
The baseline is still the latest accepted-code `large-repo --branches`
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `large-repo --branches` | 24.23s | `git::merge_detection::detect_merged_branches` | 9.99s | 27 `detect_merged_graph_descendant_of` calls consumed 9.98s |
| `large-repo --branches` | 24.23s | `git::branch::collect_branch_metadata_merge_base` | 13.85s | This is separate from merged-branch detection and remains a hotspot |
...
| `large-repo --branches` full dump | 24.23s | 14.90s | -38.5% | `list_branches` span moved from 24.0s to 13.7s |
| `collect_branch_metadata_merge_base` | 13.85s | 13.59s | unchanged | This is now the dominant local branch cost |
| `remote-heavy-repo --remotes` full dump | 60s cap | 60s cap | unchanged | Guardrail remained capped during remote squash checking |

### Outcome

Accepted. The changed function produced a clear function-level and user-visible
improvement on `large-repo --branches`, with no new remote guardrail regression.
...
For `large-repo --branches`, `git::branch::collect_branch_metadata` is now almost
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `large-repo --branches` | 14.90s | `git::branch::collect_branch_metadata_merge_base` | 13.59s | 27 per-branch `repo.merge_base()` spans dominated the run |
| `large-repo --branches` | 14.90s | `git::merge_detection::detect_merged_branches` | 15.4ms | Already fixed in Iteration 6 |
...
`large-repo` had two shallow merge-base successes and 25 branches with no useful
...
| `large-repo --branches` full dump | 14.90s | 1.21s | -91.9% | Remaining wall time is mostly the failing PR lookup (`fetch_open_prs` 710ms) |
| `remote-heavy-repo --remotes` full dump | 60s cap | 60s cap | unchanged | Guardrail remained capped during remote squash checking |

### Outcome

Accepted. The changed function removed the last multi-second local branch graph
span and made `large-repo --branches` effectively sub-second before PR lookup.
...
For `large-repo --branches`, the remaining user-visible time is now mostly PR
lookup failure handling (`fetch_open_prs_checked` around 710ms in the accepted
after-run). For `remote-heavy-repo --remotes`, the bottleneck remains remote squash
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `large-repo --branches` | 2.92s | `git::branch::list_branches` | 1.85s | `fetch_open_prs_checked` failed in 691ms; branch graph work stayed fast |
| `large-repo --worktrees` | 14.28s | `git::worktree::enrich_worktrees_worker` | 14.2s | Guardrail; still dominated by concurrent `git status --porcelain` calls |
| `remote-heavy-repo --remotes` | 60.03s cap | `git::squash_loader::squash_candidate` | 38.44s total busy | 221 real squash checks completed before the cap; mean 173.9ms, p95 372ms |
...
| Repo / view | Before | After | Delta | Evidence |
| --- | ---: | ---: | ---: | --- |
| `remote-heavy-repo squash_candidate` completed checks | 221 | 254 | +14.9% | Same 60s cap; candidate count remained 16,056 |
| `remote-heavy-repo squash_candidate` mean | 173.9ms | 155.0ms | -10.9% | Total busy time stayed around 39s because the capped run completed more checks |
| `remote-heavy-repo squash_candidate` p95 | 372ms | 226ms | -39.2% | Same log parser and same candidate order |
| Common first 221 checks | 173.9ms mean | 149.6ms mean | -24.3ms/check | 117 checks improved, 100 worsened |
| `remote-heavy-repo --remotes` full dump | 60.03s cap | 60.03s cap | unchanged | Still capped; this is a throughput improvement, not completion yet |
| `large-repo --branches` guardrail | 2.92s | 3.45s | within variance | Changed path is not used; PR lookup failure remained ~688ms |
| `large-repo --worktrees` guardrail | 14.28s | 12.52s | within variance | Changed path is not used |
...
For `remote-heavy-repo --remotes`, the next meaningful target is still
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `remote-heavy-repo --remotes` | 60.03s cap | `git::squash_loader::squash_candidate` | 39.37s total busy | 254 real squash checks completed before the cap; mean 155.0ms, p95 226ms |
...
| Repo / view | Before | After | Delta | Evidence |
| --- | ---: | ---: | ---: | --- |
| `remote-heavy-repo squash_candidate` completed checks | 254 | 260 | +2.4% | Same 60s cap; candidate count remained 16,056 |
| `remote-heavy-repo squash_candidate` mean | 155.0ms | 151.9ms | -2.0% | Small improvement on the completed checks |
| `remote-heavy-repo squash_candidate` p95 | 226ms | 217ms | -4.0% | Same parser and same candidate order |
| Common first 254 checks | 155.0ms mean | 151.3ms mean | -3.7ms/check | 157 checks improved, 89 worsened |
| `remote-heavy-repo --remotes` full dump | 60.03s cap | 60.04s cap | unchanged | Still capped; throughput improved slightly |
| `large-repo --branches` guardrail | 3.45s | 2.86s | within variance | Changed path is not used; PR lookup failure remained ~737ms |
| `large-repo --worktrees` guardrail | 12.52s | 12.92s | within variance | Changed path is not used |
...
For `remote-heavy-repo --remotes`, the remaining issue is not branch ref resolution
...
| --- | ---: | ---: | --- | --- |
| `remote-heavy-repo --remotes` | 60.04s cap | 0 entries | 260 `squash_candidate` spans | All 260 were cache misses; mean 151.9ms, p95 217ms; no cache save occurred before the cap killed the process |
...
| `large-repo --branches` guardrail | 2.86s | 2.89s | within variance | Changed path is not used |
| `large-repo --worktrees` guardrail | 12.92s | 12.73s | within variance | Changed path is not used |
...
| Repo / view | Wall time | Key span | Span time | Note |
| --- | ---: | --- | ---: | --- |
| `remote-heavy-repo --remotes` | 60.01s cap | `git::branch::remote_enricher_worker` | 18.5s | `for-each-ref ... ahead-behind` 11.2s plus `branch -r --merged` 7.30s |
| `remote-heavy-repo --remotes` | 60.01s cap | `git::squash_loader::squash_candidate` | 411 candidates | 200 cache hits, 211 misses |
...
merged condition needed by this view. A direct probe on `remote-heavy-repo` matched
...
| `remote-heavy-repo --remotes` full dump | 60.01s cap | 60.02s cap | unchanged | Still capped during squash checking |
| `remote-heavy-repo squash_candidate` progress | 411 candidates | 597 candidates | +45.3% | After run loaded 400 cache entries, so this combines cache progress with the faster remote enricher |
| `large-repo --branches` guardrail | 2.89s | 2.95s | within variance | Changed path is not used |
| `large-repo --worktrees` guardrail | 12.73s | 13.62s | within variance | Changed path is not used |
...
- Direct equivalence probe on `remote-heavy-repo`: `ahead == 0` count 546,
...
| Repo / view | Wall time | Cache load | Key span | Note |
| --- | ---: | ---: | --- | --- |
| `remote-heavy-repo --remotes` | 60.02s cap | 400 entries | 597 `squash_candidate` spans | 400 cache hits, 197 misses; no cache save occurred because the 200-insert interval was not reached |
...
| `large-repo --branches` guardrail | 2.95s | 2.88s | within variance | Changed path is not used |
| `large-repo --worktrees` guardrail | 13.62s | 14.29s | within variance | Changed path is not used |
